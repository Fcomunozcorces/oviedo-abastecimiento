...
function buildAnalysisPrompt(res) {
  ...
  const matrixRows = ["AX","AY","AZ","BX","BY","BZ","CX","CY","CZ"]
    .filter(k => matrix[k])
    .map(k => `${k} | ${matrix[k].count} SKUs | Venta: $${fmtN(Math.round(matrix[k].venta))} | Margen: $${fmtN(Math.round(matrix[k].margen))}`)
    .join("\n");

  const quiebresRows = quiebres.slice(0, 20).map(s =>
    `${s.cod} | ${s.desc.slice(0,35)} | ${s.abc}${s.xyz} | Stock:${s.stock.toFixed(0)} | Vta/día:${s.avgDaily.toFixed(1)} | Venta2026:$${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const potRows = potQuiebres.slice(0, 20).map(s =>
    `${s.cod} | ${s.desc.slice(0,35)} | ${s.abc}${s.xyz} | Stock:${s.stock.toFixed(0)} | Cob:${s.cobertura.toFixed(1)}d | Vta/día:${s.avgDaily.toFixed(1)} | Venta2026:$${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const sobrestockRows = sobrestock.slice(0, 20).map(s =>
    `${s.cod} | ${s.desc.slice(0,35)} | ${s.abc}${s.xyz} | Stock:${s.stock.toFixed(0)} | Cob:${s.cobertura === 9999 ? "∞" : s.cobertura.toFixed(0)}d | Venta2026:$${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const ocRows = ocItems.slice(0, 20).map(s =>
    `${s.cod} | ${s.desc.slice(0,35)} | ${s.abc}${s.xyz} | ${s.alert} | CantSug:${s.cantSugerida} | Vta/día:${s.avgDaily.toFixed(1)}`
  ).join("\n");

  return `Eres un Analista de Abastecimiento Senior de Oviedo Ferreterías (ferretería chilena, 5 tiendas).
Los datos ya fueron procesados en el browser. Aquí tienes los resultados calculados. Tu trabajo es INTERPRETAR, PRIORIZAR y RECOMENDAR ACCIONES CONCRETAS. No recalcules — usa los datos tal como están.

═══ KPIs GENERALES ═══
- Total SKUs catálogo: ${fmtN(nSkus)}
- SKUs activos (con venta): ${fmtN(nActivos)}
- Período analizado: ${nDays} días hábiles (2026)
- Venta total 2026: $${fmtN(Math.round(totalVenta))}
- Margen total 2026: $${fmtN(Math.round(totalMargen))} (${margenPct.toFixed(1)}%)
- Base cobertura: últimos ${nDays} días disponibles

═══ MATRIZ ABC×XYZ ═══
(ABC = % ventas acumulado: A=80%/B=15%/C=5% | XYZ = % margen acumulado: X=80%/Y=15%/Z=5%)
${matrixRows}

═══ QUIEBRES ACTIVOS (stock ≤ 0 con demanda) — ${quiebres.length} SKUs ═══
${quiebresRows || "Ninguno"}
${quiebres.length > 20 ? `... y ${quiebres.length - 20} más` : ""}

═══ QUIEBRES POTENCIALES (cobertura < 7 días) — ${potQuiebres.length} SKUs ═══
${potRows || "Ninguno"}
${potQuiebres.length > 20 ? `... y ${potQuiebres.length - 20} más` : ""}

═══ SOBRESTOCK (cobertura > 90 días) — ${sobrestock.length} SKUs ═══
${sobrestockRows}
${sobrestock.length > 20 ? `... y ${sobrestock.length - 20} más` : ""}

═══ OC SUGERIDA (clases A y B, 45 días cobertura) — ${ocItems.length} SKUs ═══
${ocRows || "Ninguno"}

INSTRUCCIONES DE RESPUESTA:
Entrega en este orden exacto:
1. RESUMEN EJECUTIVO — 5-6 KPIs en tabla, semáforo de salud del inventario
2. ANÁLISIS MATRIZ ABC×XYZ — qué significa la distribución, qué llama la atención
3. ALERTAS CRÍTICAS — quiebres y quiebres potenciales, priorizados por AX→AY→BX. Tabla con columnas: SKU | Descripción | Clase | Cobertura | Acción inmediata
4. SOBRESTOCK — top 10 por valor inmovilizado, qué hacer con cada uno
5. OC SUGERIDA — tabla ejecutiva lista para enviar a proveedor
6. 5 ACCIONES ESTA SEMANA — concretas, con responsable sugerido y fecha límite

Responde en español. Directo y ejecutivo. Usa tablas markdown.`;
}
...
El usuario ya tiene el análisis calculado. Responde preguntas de seguimiento con precisión.`;
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono','Courier New',monospace", background: C.bg, minHeight: "100vh", color: C.text, display: "flex", flexDirection: "column", fontSize: 13 }}>
      ...
      <style>{`@keyframes pulse{0%,80%,100%{opacity:.2;transform:scale(.8)}40%{opacity:1;transform:scale(1)}}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#2a2a2a;border-radius:2px}`}</style>
    </div>
  );
}
