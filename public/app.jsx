...
function buildAnalysisPrompt(res) {
  ...
  const quiebresRows = quiebres.slice(0, 10).map(s =>
    `${s.cod} | ${s.desc.slice(0,30)} | ${s.abc}${s.xyz} | Stock:${s.stock.toFixed(0)} | Vta/día:${s.avgDaily.toFixed(1)} | $${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const potRows = potQuiebres.slice(0, 10).map(s =>
    `${s.cod} | ${s.desc.slice(0,30)} | ${s.abc}${s.xyz} | Cob:${s.cobertura.toFixed(1)}d | Vta/día:${s.avgDaily.toFixed(1)} | $${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const sobrestockRows = sobrestock.slice(0, 10).map(s =>
    `${s.cod} | ${s.desc.slice(0,30)} | ${s.abc}${s.xyz} | Cob:${s.cobertura === 9999 ? "∞" : s.cobertura.toFixed(0)}d | $${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const ocRows = ocItems.slice(0, 10).map(s =>
    `${s.cod} | ${s.desc.slice(0,30)} | ${s.abc}${s.xyz} | CantSug:${s.cantSugerida} | Vta/día:${s.avgDaily.toFixed(1)}`
  ).join("\n");
...
INSTRUCCIONES: Responde en español, directo y ejecutivo, con tablas markdown.
1. RESUMEN EJECUTIVO — KPIs clave en tabla
2. ALERTAS CRÍTICAS — quiebres priorizados AX→AY→BX con acción inmediata
3. SOBRESTOCK — top 5 con acción concreta
4. OC SUGERIDA — tabla lista para enviar
5. 3 ACCIONES ESTA SEMANA — responsable y fecha`;
}
