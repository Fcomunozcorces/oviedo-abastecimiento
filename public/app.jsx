...
function buildAnalysisPrompt(res) {
  ...
  const matrixRows = ["AX","AY","AZ","BX","BY","BZ","CX","CY","CZ"]
    .filter(k => matrix[k])
    .map(k => `${k}|${matrix[k].count} SKUs|$${fmtN(Math.round(matrix[k].venta))}|$${fmtN(Math.round(matrix[k].margen))}`)
    .join("\n");

  const quiebresRows = quiebres.slice(0, 10).map(s =>
    `${s.cod}|${s.desc.slice(0,28)}|${s.abc}${s.xyz}|${s.avgDaily.toFixed(1)}/día|$${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const potRows = potQuiebres.slice(0, 10).map(s =>
    `${s.cod}|${s.desc.slice(0,28)}|${s.abc}${s.xyz}|${s.cobertura.toFixed(0)}d|${s.avgDaily.toFixed(1)}/día`
  ).join("\n");

  const sobrestockRows = sobrestock.slice(0, 10).map(s =>
    `${s.cod}|${s.desc.slice(0,28)}|${s.abc}${s.xyz}|${s.cobertura===9999?"∞":s.cobertura.toFixed(0)}d|$${fmtN(Math.round(s.venta))}`
  ).join("\n");

  const ocRows = ocItems.slice(0, 10).map(s =>
    `${s.cod}|${s.desc.slice(0,28)}|${s.abc}${s.xyz}|${s.cantSugerida} uds|${s.avgDaily.toFixed(1)}/día`
  ).join("\n");

  return `Analista Senior Oviedo Ferreterías. Responde en español con tablas markdown. Sé ejecutivo y directo.

KPIs: ${fmtN(nSkus)} SKUs | Activos: ${fmtN(nActivos)} | Venta $${fmtN(Math.round(totalVenta))} | Margen ${margenPct.toFixed(1)}% | ${nDays} días

MATRIZ ABC×XYZ (SKU|count|venta|margen):
${matrixRows}

QUIEBRES ACTIVOS — ${quiebres.length} SKUs (top 10):
${quiebresRows || "Ninguno"}

QUIEBRES POTENCIALES <7d — ${potQuiebres.length} SKUs (top 10):
${potRows || "Ninguno"}

SOBRESTOCK >90d — ${sobrestock.length} SKUs (top 10):
${sobrestockRows}

OC SUGERIDA — ${ocItems.length} SKUs (top 10):
${ocRows || "Ninguno"}

Entrega en este orden:
1. RESUMEN EJECUTIVO — tabla con KPIs clave y semáforo de salud
2. ALERTAS CRÍTICAS — tabla quiebres priorizados por AX→AY→BX, acción inmediata por cada uno
3. SOBRESTOCK — tabla top 10, acción concreta por cada uno
4. OC SUGERIDA — tabla lista para enviar a proveedor
5. 3 ACCIONES ESTA SEMANA — con responsable y fecha límite`;
}
